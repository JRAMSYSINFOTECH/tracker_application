...
function removeTimeOverlaps(plan) {
  ...
  for (let i = 1; i < plan.length; i++) {
    ...
    if (currStart < prevEnd) {
      ...
      curr.end = new Date(prevEnd.getTime() + duration);
    }
  }
...
async function callAIWithRetry(prompt, retries = 2) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const plan = Array.isArray(parsed) ? parsed : parsed.schedule;
...
function calculateConfidence(task, isAI) {
  ...
  if (hoursLeft < 24) score += 0.3;
  else if (hoursLeft < 72) score += 0.2;

  if (task.importance_hint === "high") score += 0.2;
  else if (task.importance_hint === "medium") score += 0.1;
...
export const generatePlan = async (req, res) => {
  ...
  try {
    ...
    if (tasks.length === 0) {
      return res.status(400).json({ message: "No pending tasks" });
    }
    ...
    try {
      const prompt = `
STRICT RULES:
- Each task_id must appear ONLY ONCE
...
Format:
[
 { "task_id": number, "start": "ISO", "end": "ISO" }
]
`;

      aiPlan = await callAIWithRetry(prompt, 2);
      ...
      aiPlan = tasks.map(task => {
        ...
        end.setMinutes(end.getMinutes() + (task.estimated_minutes || 60));
    ...
    let filteredPlan = aiPlan.filter(item => {
    ...
    // ================= FIX OVERLAPS =================
    filteredPlan = removeTimeOverlaps(filteredPlan);

    // ================= SAVE =================
    const planItems = filteredPlan.map((item, index) => {
      ...
      return {
        ...
        confidence_score: calculateConfidence(task, usedAI)
      };
    });

    await prisma.dailyPlanItem.createMany({ data: planItems });
    ...
    const finalPlan = await prisma.dailyPlan.findUnique({
      where: { plan_id: plan.plan_id },
      include: {
        items: {
          include: { task: true },
    ...
    const total = finalPlan.items.length;

    const completed = finalPlan.items.filter(
